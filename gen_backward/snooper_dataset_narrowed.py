...
class Snooper:
    ...
    def snoop(self, x, y, y_target, pattern_init, mask_init,poison_type,poison_loc,poison_size):
        ...
        for step in range(steps):
            ...
            loss_reg_list = []
            loss_list = []
            for idx in trange(ceil(narrow_index*ceil((len(x)/32)-1))):
            ...
            if avg_loss < loss_best:
                ...
                loss_best = avg_loss
                with open('backward_triggers_dataset_narrowed/pattern_%s_%s_%d_%f.npy'%(poison_type,poison_loc,poison_size,narrow_index), 'wb') as f:
                    np.save(f, pattern_best)
                with open('backward_triggers_dataset_narrowed/mask_%s_%s_%d_%f.npy'%(poison_type,poison_loc,poison_size,narrow_index), 'wb') as f:
                    np.save(f, mask_best)
...
if __name__ == '__main__':
    ...
    #args.checkpoint returns the route of the hdf5 file
    infos=args.checkpoint.split('-')
    poison_type=infos[1]
    poison_loc=infos[2]
    ...
    with open('backward_triggers_dataset_narrowed/pattern_%s_%s_%d.npy'%(poison_type,poison_loc,poison_size), 'wb') as f:
        np.save(f, pattern_best)
    with open('backward_triggers_dataset_narrowed/mask_%s_%s_%d.npy'%(poison_type,poison_loc,poison_size), 'wb') as f:
        np.save(f, mask_best)
